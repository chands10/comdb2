...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
int sqlite3DbMaskAllZero(yDbMask m, int start){
  int sz = (SQLITE_MAX_ATTACHED + 9) / 8;
  int i;
  for(i=start; i<sz; i++) if( m[i] ) return 0;
  return 1;
}
...
*/
#if defined(SQLITE_BUILDING_FOR_COMDB2)
static Table *sqlite3FindTable_int(sqlite3 *db, const char *zName, const char *zDatabase){
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
static Table *sqlite3FindTable_main(
  sqlite3 *db,
  const char *zName,
...
** prevent cached remote tables with no database name,
** pre-populated sqlite engines may have malformed queries
*/
  p = sqlite3FindTable_int(db, zName, dbName);
  if( p && !zDatabase ){
    p = NULL;
    goto done;
  }
...
Table *sqlite3FindTable(sqlite3 *db, const char *zName, const char *zDatabase){
   return sqlite3FindTable_main(db, zName, zDatabase, 1, 0, 0);
}
...
Table *sqlite3FindTableCheckOnly(
  ...
  const char *zDatabase
   ...
   return sqlite3FindTable_main(db, zName, zDatabase, 0, 0, 0);
}
...
Table *sqlite3FindTableCheckOnlyNoAlias(
  ...
  const char *zDatabase
   ...
   return sqlite3FindTable_main(db, zName, zDatabase, 0, 0, 1);
}
...
Table *sqlite3FindTableByAnalysisLoad(
  ...
  return sqlite3FindTable_main(db, zName, zDatabase, 1, 1, 0);
}
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
*/
void sqlite3ResetOneSchemaByName(sqlite3 *db, const char *zName, const char *zDatabase){
  int i;
  int len, len2;
  ...
  for(i=0; i<db->nDb; i++){
    ...
    if( pDb && pDb->zDbSName){
      ...
      len2 =  sqlite3Strlen30(pDb->zDbSName);
  ...
  if( !IN_SPECIAL_PARSE ){
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
    pTable = sqlite3FindTableCheckOnly(db, zName, zDb);
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
    ...
    if( pTable ){
      if( !noErr ){
        ...
        if ( strncmp(zName, "sqlite_stat", 11)!=0 ){
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
#if !defined(SQLITE_BUILDING_FOR_COMDB2)
      /* COMDB2: No "break", "INT" COLLIDES WITH "INTERVAL", 10252007dh */
      break;
#endif /* !defined(SQLITE_BUILDING_FOR_COMDB2) */
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  ...
  if( !IN_RENAME_OBJECT ){
    ...
    */
    ...
    if( db->init.busy ){
      ...
      if( db->init.iDb>1 ){
        extern int gbl_fdb_track;
        if (gbl_fdb_track)
          logmsg(LOGMSG_USER, "XXX: no skip-scan for remote index %s:%s\n", 
    ...
    */
      ...
      */
      sqlite3NestedParse(pParse,
#if defined(SQLITE_BUILDING_FOR_COMDB2)
         "INSERT INTO %Q." LEGACY_SCHEMA_TABLE " VALUES('index',%Q,%Q,#%d,%Q,NULL);",
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  /* AZ: allow unique indexes to have cost of 1 */
    ...
    for(i=0, pItem=pList->a; i<pList->nSrc; i++, pItem++){
      ...
      if( pItem->fg.isSubquery ){
        ...
        sqlite3SrcListAssignCursors(pParse, pItem->u4.pSubq->pSelect->pSrc,
            is_recording ||
            pItem->u4.pSubq->pSelect->recording);
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
        sqlite3SrcListAssignCursors(pParse, pItem->u4.pSubq->pSelect->pSrc);
      }
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
  ...
  unsigned long long colMask)
{
  ...
  if( pIdx->aiColumn[0] == XN_EXPR ){
     pExprDesc = sqlite3ExprDescribe(db->pVdbe, pIdx->aColExpr->a[0].pExpr);
     if( pExprDesc == NULL ){
         return NULL;
     }
     isExpr = 1;
  ...
  for(i=1; i< pIdx->nKeyCol; i++){
    ...
    if( pIdx->aiColumn[i] == XN_EXPR ){
      pExprDesc = sqlite3ExprDescribe(db->pVdbe, pIdx->aColExpr->a[i].pExpr);
      isExpr = 1;
    }else
    ...
    if( !done_key ){
      if( (*hasCondition) && i < nfields ){
        ...
        /*fprintf(stderr, "\"%s\" has condition \"%s\"\n",
            isExpr?pExprDesc:colName, retCond);*/
      }
