...
comments enclosed in ```/* ... */``` are also supported. Lines end with a
LF character (0x0A); CR+LF pairs (0x0D 0x0A) are not supported.
...
To only copy a subset of the columns, follow the ```datacopy``` keyword by a comma-separated list of columns 
closed by parentheses to include in the datacopy.
