...
void reset_aa_counter(char *tblname)
{
    ...
    if (sizeof(tbl->aa_needs_analyze_time) == 8)
        XCHANGE64(tbl->aa_needs_analyze_time, 0);
    else if (sizeof(tbl->aa_needs_analyze_time) == 4)
        XCHANGE32(tbl->aa_needs_analyze_time, 0);
    else
        abort();
    ...
    if (save_freq > 0 && thedb->master == gbl_myhostname) {
        ...
        char epoch[30] = {0};
        sprintf(epoch, "%ld", (int64_t)tbl->aa_lastepoch);
    ...
    ctrace("AUTOANALYZE: Analyzed Table %s, reseting counter to %d, last "
           ...
           ctime_r(&tbl->aa_lastepoch, my_buf), tbl->aa_needs_analyze,
           ctime_r(&tbl->aa_needs_analyze_time, my_buf2));
}
...
static void get_saved_counter_epoch_queue(char *tblname, unsigned *aa_counter,
                                          time_t *aa_lastepoch, unsigned *aa_needs_analyze,
                                          time_t *aa_needs_analyze_time)
{
    int rc;
    if (aa_counter) {
        *aa_counter = 0;
    ...
    if (aa_needs_analyze_time) {
        ...
        *aa_needs_analyze_time = 0;
        ...
        if (rc == 0) {
            *aa_needs_analyze_time = atoi(needs_analyze_time_str);
...
int load_auto_analyze_counters(void)
{
    ...
    for (int i = 0; i < thedb->num_dbs; i++) {
        ...
        if (save_freq > 0) {
            ...
            time_t needs_analyze_time = 0;
            get_saved_counter_epoch_queue(tbl->tablename, &saved_counter, &tbl->aa_lastepoch, &needs_analyze, &needs_analyze_time);
            ...
            if (sizeof(tbl->aa_needs_analyze_time) == 8)
                ...
                XCHANGE32(tbl->aa_needs_analyze_time, needs_analyze_time);
            else
                abort();
            ...
            ctrace("AUTOANALYZE: Loading table %s, count %d, last run time %s, needs analyze %d, needs analyze time %s",
                   tbl->tablename, tbl->aa_saved_counter, ctime_r(&tbl->aa_lastepoch, my_buf), tbl->aa_needs_analyze,
                   ctime_r(&tbl->aa_needs_analyze_time, my_buf2));
        }
    }
...
void get_auto_analyze_tbl_stats(struct dbtable *tbl, int include_updates, int *delta, int *saved_counter,
                                unsigned int *newautoanalyze_counter, double *new_aa_percnt)
{
    ...
    *delta = curr - prev;
    *saved_counter = ATOMIC_LOAD32(tbl->aa_saved_counter);
...
void stat_auto_analyze(void)
{
    ...
    for (int i = 0; i < thedb->num_dbs; i++) {
        ...
        time_t needs_analyze_time;
        if (sizeof(tbl->aa_needs_analyze_time) == 8)
            needs_analyze_time = ATOMIC_LOAD64(tbl->aa_needs_analyze_time);
        else if (sizeof(tbl->aa_needs_analyze_time) == 4)
            needs_analyze_time = ATOMIC_LOAD32(tbl->aa_needs_analyze_time);
        else
            abort();
            ...
            if (bdb_attr_get(thedb->bdb_attr, BDB_ATTR_AA_REQUEST_MODE)) {
                if (!needs_analyze) {
                    ...
                    if (save_freq > 0) {
                        ...
                        sprintf(needs_analyze_time_str, "%ld", (int64_t)needs_analyze_time);
                ...
                ctrace(
                    ...
                    tbl->tablename, newautoanalyze_counter, ctime_r(&tbl->aa_lastepoch, my_buf), needs_analyze, ctime_r(&needs_analyze_time, my_buf2));
