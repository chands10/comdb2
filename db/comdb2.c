...
int gbl_debug_pb_connectmsg_physrep_check = 0;
...
int64_t gbl_legacy_requests_all = 0; /* includes all auxiliary request types */
...
void *statthd(void *p)
{
    ...
    while (!db_is_exiting()) {
        ...
        if (diff_qtrap || diff_nsql || diff_newsql || diff_nsql_steps ||
            ...
            diff_conns || diff_conn_timeouts || curr_conns) {
            if (gbl_report) {
                ...
                if (curr_conns)
                    logmsg(LOGMSG_USER, " current_connects %"PRId64, curr_conns);
