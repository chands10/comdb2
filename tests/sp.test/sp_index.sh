...
if ! cdb2sql -s testdb local -- "create table tickets {$(cat tickets.csc2)}" 2>&1 1>/dev/null; then
  true
fi
...
cdb2sql -s testdb local -- "drop table tickets"
if ! drop_func_proc; then true; fi
cdb2sql -s testdb local -- "drop index id_pending_resolved on ticks"
drop_func_proc
cdb2sql -s testdb local -- "drop table ticks"
