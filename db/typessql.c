...
int typessql_next_row(struct sqlclntstate *clnt, sqlite3_stmt *stmt)
{
    ...
    if (typessql_state->first_run && gbl_typessql_records_max > 0) {
        ...
        while (null_col_exists && typessql_state->size < gbl_typessql_records_max) {
            ...
            int rc = bdb_temp_table_insert(thedb->bdb_env, typessql_state->cur, &count, sizeof(count),
                            packed, packed_size, &bdberr);
            if (rc) {
                logmsg(LOGMSG_ERROR,
        ...
        if (null_col_exists && typessql_state->size > 0) // only print if results set is non-empty
            logmsg(LOGMSG_WARN, "%s: Unable to find all non-null types for query %s.\n", __func__, clnt->sql);
    }

    if (typessql_state->have_current_row) {
        free_row(&typessql_state->current_row, typessql_state->ncols);
        typessql_state->current_row.packed = NULL;
        typessql_state->current_row.unpacked = NULL;
        typessql_state->current_row.db = NULL;
        typessql_state->current_row.row_size = 0;
...
/*
TODO:
What if calling run_stmt again while already have typessql_state (like fdb_cursor_move_sql). Currently freeing and reallocating memory for typessql
